...
- ال Accessors: يُستخدم لتعديل قيمة الحقل عند قراءتها من قاعدة البيانات - مثال: لو الاسم مخزن بحروف صغيرة في قاعدة البيانات، وتريدين عرضه بحروف كبيرة - يشتغل عند قراءة البيانات (get).
- ال Mutators: يُستخدم لتعديل قيمة الحقل قبل حفظها في قاعدة البيانات - مثال: تريدين حفظ الاسم دائماً بحروف صغيرة - يشتغل عند حفظ البيانات (set). 
```php
// casts
protected $casts = [
    'is_active' => 'boolean',
...
Response
```
1- دخول الطلب إلى التطبيق - جميع الطلبات تدخل من خلال الملف 
```php
public/index.php
```
2- تحميل التطبيق (Bootstrap) - يتم تحميل إطار العمل وتجهيز الخدمات الأساسية

3- إنشاء Application Instance - يتم إنشاء كائن التطبيق الموجود في 
```php
bootstrap/app.php
```
4- تشغيل Middleware - يتم تنفيذ الـ Middleware للتحقق من أمور مثل:
هل المستخدم مسجل دخول؟
هل الطلب يحتوي على Token صحيح؟

5- توجيه الطلب (Routing)

6- تنفيذ Controller

7- التعامل مع قاعدة البيانات - model

8- إرجاع Response - يتم إرجاع View
